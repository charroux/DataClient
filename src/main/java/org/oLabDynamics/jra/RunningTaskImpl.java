...
import jra.exec.TimeUnit;
import jra.exec.RunningTask.State;
import jra.exec.RunningTask.UPDATE_STATE_FREQUENCY;
 ...
 *
 */
public class RunningTaskImpl extends org.oLabDynamics.jra.rest.Resource implements RunningTask, Runnable{
	...
	@Override
	public synchronized List<OutputData> getResult() throws InterruptedException, JRAException{
		return this.getResult(UPDATE_STATE_FREQUENCY.NORMAL);
	}
	
	@Override
	public List<OutputData> getResult(UPDATE_STATE_FREQUENCY frequency) throws InterruptedException, JRAException {
		...
		if(state == State.FAILED){
			throw new JRAException(this.failureInfo);
		}
		
		if(state == State.CANCELLED_BY_USER){
			throw new JRAException("Cancelled by user");
		}
		
		if(state == State.KILLED_BY_ADMINISTRATOR){
			throw new JRAException("Killed by administrator");
		}
	...
	@Override
	public List<OutputData> getResult(long timeout, TimeUnit unit) throws InterruptedException, JRAException, jra.exec.TimeoutException{
		return null;
	}
